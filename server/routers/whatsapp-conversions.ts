...
export const whatsappConversionsRouter = router({
  /**
   * Track an ad impression (public - no auth required)
   */
  trackImpression: publicProcedure
    ...
    .mutation(async ({ input, ctx }) => {
      ...
      return await db.trackAdImpression(impressionData);
    }),

  /**
   * Get ad impression statistics (admin only)
   */
  getImpressionStats: protectedProcedure
    .input(z.object({
      dateFilter: z.enum(['7d', '30d', 'all']).optional(),
    }).optional())
    .query(async ({ input, ctx }) => {
      ...
      return await db.getAdImpressionStats(input?.dateFilter);
    }),
  /**
  ...
  /**
   * Get recent ad impressions (admin only)
   */
  getRecentImpressions: protectedProcedure
    ...
    .query(async ({ input, ctx }) => {
      ...
      return await db.getRecentAdImpressions(input?.limit);
    }),
