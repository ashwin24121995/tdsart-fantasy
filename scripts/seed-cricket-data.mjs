...
import { sports, players, contests, achievements } from "../drizzle/schema.js";
...
async function seedData() {
  ...
  const cricketSport = await db.select().from(sports).where({ slug: "cricket" }).limit(1);
  const sportId = cricketSport[0].id;
